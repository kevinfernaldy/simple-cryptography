...
#include "ui_vigenereautokeywidget.h"
...
VigenereAutoKeyWidget::VigenereAutoKeyWidget(QWidget *parent) : QWidget(parent), ui(new Ui::VigenereAutoKeyWidget)
{
    ui->setupUi(this);
    ...
    ui->textTypeContainer_3->setVisible(false);
...
VigenereAutoKeyWidget::~VigenereAutoKeyWidget() {
    delete ui;
    if (cipher != nullptr) delete cipher;
}

void VigenereAutoKeyWidget::encrypt(std::string key, std::string plainText)
{
    if (cipher != nullptr) delete cipher;
    cipher = new VigenereAutoKeyCipher(key);
...
void VigenereAutoKeyWidget::decrypt(std::string key, std::string cipherText)
{
    if (cipher != nullptr) delete cipher;
    cipher = new VigenereAutoKeyCipher(key);
...
std::string VigenereAutoKeyWidget::readFile(std::string fileName)
{
...
void VigenereAutoKeyWidget::handleFileOutput(std::string fileName, std::string content)
{
...
void VigenereAutoKeyWidget::on_encryptButton_clicked()
{
    ui->plainTextWarningLabel->setVisible(false);
...
void VigenereAutoKeyWidget::on_decryptButton_clicked()
{
    ui->plainTextWarningLabel->setVisible(false);
...
void VigenereAutoKeyWidget::on_renderCipherTextBox_stateChanged(int arg1)
{
    switch(arg1) {
        case 2: {
...
void VigenereAutoKeyWidget::on_sourceComboBox_currentIndexChanged(int index)
{
    switch(index) {
        case 0: {
            ui->textTypeContainer_3->setVisible(false);
        ...
        case 1: {
            ui->textTypeContainer_3->setVisible(true);
...
void VigenereAutoKeyWidget::on_browseFileInputButton_clicked()
{
...
void VigenereAutoKeyWidget::on_browseFileOutputButton_clicked()
{
    QFileDialog *dialog = new QFileDialog;
